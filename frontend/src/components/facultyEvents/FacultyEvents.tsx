...
export default function FacultyEvents() {
  const [setNavElements, setHeaderMessage]: FacultyDashboardPageMethods =
    useOutletContext();
  useEffect(() => {
    setNavElements((prev: NavElements[]) =>
      prev.map((e: NavElements) => ({
        ...e,
        active: e.displayName === "Events" ? true : false,
      }))
    );
