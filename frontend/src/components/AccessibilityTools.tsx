...
export default function AccessibilityTools(){
   ...
   return(
      <div className="flex bg-slate-200 rounded border border-gray-300 text-sm dark:bg-slate-900">
         ...
         {theme === "light" && <button className="px-3 rounded-r bg-black text-white hover:bg-gray-700" onClick={()=> setTheme("dark")}>A</button>}
         {theme === "dark" && <button className="px-3 border-r border-gray-300 bg-white hover:bg-muted" onClick={()=> setTheme("light")}>A</button>}
      </div>
   )
}